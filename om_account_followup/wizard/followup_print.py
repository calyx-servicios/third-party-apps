...
class FollowupPrint(models.TransientModel):
    ...
    followup_id = fields.Many2one('followup.followup', 'Follow-Up',
                                  required=True,default=_get_followup)
    ...
    def _get_partners_followp(self):
        ...
        for partner_id, followup_line_id, date_maturity, date, id in \
            ...
            if date_maturity:
                followup_line_date = fups[followup_line_id][0].strftime(
                        '%Y-%m-%d')
                if date_maturity <= datetime.date(int(followup_line_date[:4]),int(followup_line_date[5:7]),int(followup_line_date[8:10])):
                    if stat_line_id not in partner_list:
                        partner_list.append(stat_line_id)
                    to_update[str(id)] = {'level': fups[followup_line_id][1],
                                          'partner_id': stat_line_id}
            elif date and date <=  datetime.date(int(fups[followup_line_id][0].strftime('%Y')),int(fups[followup_line_id][0].strftime('%m')),int(fups[followup_line_id][0].strftime('%d'))):
                if stat_line_id not in partner_list:
                    partner_list.append(stat_line_id)
                to_update[str(id)] = {'level': fups[followup_line_id][1],
                                      'partner_id': stat_line_id}
