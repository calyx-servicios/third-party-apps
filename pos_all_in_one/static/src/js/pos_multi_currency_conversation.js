odoo.define('pos_all_in_one.pos_multi_currency_conversation', function (require) {
"use strict";
		...
		change_currency: function() {
			...
			for(var i=0;i<currencies.length;i++)
			{
				if(cur != pos_currency.id && cur==currencies[i].id)
				{
					...
					var curr_tot =order.get_due()*currency_in_pos;
		...
		update_amount: function() {
			...
			if(selected_paymentline){
				for(var i=0;i<currency.length;i++)
				{
					...
					{
						let c_rate = self.pos.company_currency.rate/currency[i].rate;
						...
						this.$('.show-payment').text(this.format_currency_no_symbol(selected_paymentline.amount));
						selected_paymentline.currency_name =currency[i].name;
						selected_paymentline.currency_amount = selected_paymentline.amount_currency;
					}
				}
			}
