odoo.define('pos_all_in_one.bi_pos_reprint_reorder', function(require) {
	"use strict";
	...
	var pos_orders_list = require('pos_all_in_one.pos_orders_list');
	...
	pos_orders_list.include({
		...
		show: function(options) {
			...
			self.$('.orders-list-contents').delegate('.return-order', 'click', function(result) {
				var order_id = parseInt(this.id);
				var selectedOrder = null;
				var orders =  self.pos.get('all_orders_list');
				for(var i = 0, len = Math.min(orders.length,1000); i < len; i++) {
