			...
			for line in odr.lines:					
				...
				if product in prod_data:
					...
					prod_data[product].update({
						...
						'discount':float(old_discount+(line.price_unit * float(line.qty))-float("{0:.2f}".format(line.price_subtotal))),
					})
				else:	
					prod_data.update({ product : {
						...
						'qty' : float(line.qty),
						'discount':(line.price_unit * float(line.qty))-float("{0:.2f}".format(line.price_subtotal)),						
					}})
